(() => {
  ...
  function syncStatuses(targetState) { (targetState.projects || []).forEach((project) => { const tasks = Array.isArray(project.tasks) ? project.tasks : []; if (tasks.length && tasks.every((task) => task.done)) project.status = "done"; }); }
