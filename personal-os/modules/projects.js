(() => {
  ...
  function getProgress(item) { const tasks = Array.isArray(item.tasks) ? item.tasks : []; return tasks.length ? Math.round((tasks.filter((task) => task.done).length / tasks.length) * 100) : clamp(item.progress); }
