(() => {
  ...
  function syncStatuses(targetState) { (targetState.hobbies || []).forEach((hobby) => { const checklist = Array.isArray(hobby.checklist) ? hobby.checklist : []; if (checklist.length && checklist.every((check) => check.done)) hobby.status = "done"; }); }
